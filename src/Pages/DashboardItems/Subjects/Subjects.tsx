...
import { BookOpen, GraduationCap, FlaskConical, Layers3, Search, RefreshCcw, Users, Building2, UserRound, Filter, Sparkles, Binary } from "lucide-react";
...
function Subjects() {
    ...
    const deleteSubject = async (id: string) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.message || 'Purge request failed');
        }
    };

    const stats = [
        { icon: BookOpen, title: "Curriculum Total", count: subjects.length, color: "from-indigo-600 to-indigo-500", shadow: "shadow-indigo-100" },
        { icon: GraduationCap, title: "Primary Lectures", count: subjects.filter((s: any) => s.subjectType?.toUpperCase() === "LECTURE").length, color: "from-emerald-600 to-emerald-500", shadow: "shadow-emerald-100" },
        { icon: FlaskConical, title: "Laboratory Units", count: subjects.filter((s: any) => s.subjectType?.toUpperCase() === "LAB").length, color: "from-amber-600 to-amber-500", shadow: "shadow-amber-100" },
        { icon: Binary, title: "Hybrid Modules", count: subjects.filter((s: any) => s.subjectType?.toUpperCase() === "LECTURE + LAB").length, color: "from-rose-600 to-rose-500", shadow: "shadow-rose-100" }
    ];
        ...
        <div className="flex h-screen w-screen bg-slate-50 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-full overflow-y-auto animate-page">
                <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4">
                    <ProfileNavbar content="Academic Curriculum Matrix" />
                    ...
                    <div className="premium-card !p-4 grid grid-cols-1 md:grid-cols-5 gap-4 bg-slate-50/50">
                        ...
                        <div className="relative group">
                            ...
                            <select className="input-box !py-3 !pl-11 !bg-white cursor-pointer appearance-none" value={departmentFilter} onChange={(e)=>setDepartmentFilter(e.target.value)}>
                                ...
                                {departments?.map((dept: any) => <option key={dept.departmentName} value={dept.departmentName}>{dept.departmentName}</option>)}
                            </select>
                        </div>
                                            ...
                                            <tr key={s.subjectId} className="group hover:bg-slate-50/70 transition-colors border-b border-slate-50 last:border-none">
                                                ...
                                                <td className="p-8">
                                                    <span className="px-3 py-1.5 bg-blue-50 text-blue-700 font-black text-[10px] rounded-lg tracking-widest uppercase border border-blue-100/50">
                                                        {s.teacherName?.length || 0} Assignees
                                                    </span>
                                                </td>
