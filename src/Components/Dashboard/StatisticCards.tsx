    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setCounts({
                    teachers: t.data.teachers.length || 0,
                    subjects: s.data.subjects.length || 0,
                    rooms: r.data.rooms.length || 0,
                    departments: d.data.department.length || 0,
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 px-4 sm:px-6 md:px-3 mx-3">
            {stats.map((item, index) => {
                ...
                return (
                    <div key={index} className="bg-white/5 border border-white/10 rounded-[2rem] p-6 group hover:-translate-y-1.5 transition-all duration-300 hover:bg-white/10 hover:border-indigo-500/20 shadow-xl">
                        <div className="flex flex-col h-full space-y-6">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="text-right space-y-1">
                                    ...
                                    <h4 className="text-3xl font-black text-violet-500 tracking-tight">{item.count}</h4>
                                </div>
                            </div>
